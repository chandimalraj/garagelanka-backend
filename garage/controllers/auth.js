...
exports.auth = async (req, res) => {
  ...
  try {
    // see if user exixts
    let user = await User.findOne({ mobile: mobile }).populate(
      "serviceCenterID",
      ["district_id", "city_id"]
    );
    if (!user) {
    ...
    // check for the users service center

    // redirecty tokens for owners

    let payload = {};

    if (user.userRole == "Owner") {
      // console.log("this user is a owner");
      // console.log(user.serviceCenterID);
      ...
      payload = {
        user: {
          ...
          mobile: user.mobile,
          serviceCenters: user.ownServiceCenters,
...
exports.realeaseTokenToOwners = async (req, res) => {
  const errors = validationResult(req);
